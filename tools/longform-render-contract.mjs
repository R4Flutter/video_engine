...
const MEDIA = new Set(["footage", "evidence", "stat", "compare", "chart", "timeline", "icon", "payoff"]);
const inferRenderKind = (module) => {
  if (module === "footage") return "MEDIA";
  if (module === "evidence") return "EVIDENCE";
  if (module === "stat" || module === "chart") return "DATA";
...
export function buildLongformRenderContract(plan, references = []) {
  if (plan?.project?.mode !== "LONG_FORM") return plan;
