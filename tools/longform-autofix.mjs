...
for (let i = 0; i < beats.length; i++) {
  ...
  if (d > 10 && !Array.isArray(b?.render?.motion?.internalChangeAt) || (Array.isArray(b?.render?.motion?.internalChangeAt) && b.render.motion.internalChangeAt.length === 0)) {
    const at = Number((d * 0.55).toFixed(2));
    b.render = b.render || {};
    b.render.motion = { ...(b.render.motion || {}), internalChangeAt: [at] };
    changed++; run++;
  }
  ...
  if ((b?.retention?.fatigueRisk ?? 0) >= 0.55) {
    b.render = b.render || {};
    b.render.motion = { ...(b.render.motion || {}), internalChangeAt: Array.isArray(b.render.motion?.internalChangeAt) && b.render.motion.internalChangeAt.length ? b.render.motion.internalChangeAt : [Number((d * 0.5).toFixed(2))] };
  }
}
