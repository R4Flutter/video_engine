...
const video = resolve(ROOT, "video/out/final.mp4");
...
if (Math.abs(duration - expected) > 0.35) errors.push(`duration mismatch: rendered=${duration.toFixed(2)}s plan=${expected.toFixed(2)}s`);
...
const blackMatches = [...black.stderr.matchAll(/black_start:([\d.]+)\s+black_end:([\d.]+)\s+black_duration:([\d.]+)/g)].map(m => ({ start:Number(m[1]), end:Number(m[2]), duration:Number(m[3]) }));
...
const frozenMatches = [...frozen.stderr.matchAll(/freeze_start:([\d.]+).*?freeze_end:([\d.]+).*?freeze_duration:([\d.]+)/gs)].map(m => ({ start:Number(m[1]), end:Number(m[2]), duration:Number(m[3]) }));
...
const silence = [...audio.stderr.matchAll(/silence_start: ([\d.]+)|silence_end: ([\d.]+)/g)].map(m => m[1] ? { type:"start", at:Number(m[1]) } : { type:"end", at:Number(m[2]) });
...
const report = { status: errors.length ? "FAIL" : warnings.length ? "PASS_WITH_WARNINGS" : "PASS", errors, warnings, media: { duration, width, height, sizeBytes:Number(format.size || 0), videoCodec:vstream?.codec_name || "" }, blackIntervals:blackMatches, frozenIntervals:frozenMatches, silenceEvents:silence.length, checkedAt:new Date().toISOString() };
mkdirSync(join(ROOT,"video/out"),{recursive:true});
writeFileSync(reportPath, JSON.stringify(report,null,2));
console.log(`PIXEL QC  ${report.status}`);
console.log(`  duration ${duration.toFixed(2)}s / plan ${expected.toFixed(2)}s`);
console.log(`  format   ${width}x${height} · ${vstream?.codec_name || "unknown"}`);
