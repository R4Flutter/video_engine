...
for (const m of md.matchAll(BEAT_RE)) {
  ...
  // The big overlay line. Both engines carry one: it is the beat's claim, and
  // for beat 1 it is the frame the whole video is judged on, so the director
  // needs it whichever vocabulary is staging.
  beat.text = rows["on-screen text"] ?? "";

  // ------------------------------------------------------------ direction
  // Rows that direct the *edit* rather than stage the frame. Every one is
  // optional and every one beats a heuristic, so a script that writes none of
  // them still parses — it just hands more decisions to the machine, and the
  // machine has never seen the story.
  //
  // `hook` is the row that fixes the failure that started all this: it names
  // the exact text that must be legible on frame one, complete, before any
  // animation runs. Without it the director falls back to `on-screen text`,
  // and without that to the first spoken sentence.
  const DIRECTION = {
    purpose: "purpose",
    question: "question",
    reveal: "reveal",
  ...
  for (const [key, row] of Object.entries(DIRECTION)) {
    if (rows[row]) beat[key] = rows[row];
  }
  // Seconds, so they are numbers by the time the director sees them.
  for (const [key, row] of [
    ["jcut", "j-cut"],
    ["lcut", "l-cut"],
  ]) {
    if (rows[row]) {
      const v = Number(rows[row].replace(/[^\d.]/g, ""));
      if (Number.isFinite(v) && v > 0) beat[key] = v;
    }
  }
  ...
  if (engine === "vox") {
    ...
    // The impact word: "BET @ 3.1" — a word and the second into the beat its
    // hit lands. Rendered as the slam overlay; the scene punches on the hit.
    const slam = /^([A-Z0-9$%]+)\s*@\s*([\d.]+)$/i.exec(rows["slam"] ?? "");
    if (slam) beat.slam = { word: slam[1].toUpperCase(), at: Number(slam[2]) };
...
const directed = beats.filter((b) =>
  ["purpose", "question", "reveal", "emotion", "hook", "camera", "music", "silence"].some(
    (k) => b[k],
  ),
).length;
...
console.log(
  `${dst}\n  ${engine} · ${out.width}x${out.height} · ${out.durationInSeconds}s · ` +
    ...
    `\n  directed: ${directed}/${beats.length} beats carry editorial rows` +
    (beats[0]?.hook || beats[0]?.text
      ? `\n  frame 1:  "${(beats[0].hook || beats[0].text).slice(0, 52)}"`
      : `\n  frame 1:  (no on-screen text — frame one will render blank)`),
);
