...
export const assetForBeat = (beat: ScriptBeat): AssetRecord | undefined => {
  ...
  const direct = beat.assetPath?.trim() || beat.footage?.trim();
  if (direct) {
...
export const assetIssuesForScript = (script: Script) => {
  ...
  for (const beat of script.beats) {
    const needsPhysicalAsset = REQUIRED_MODULES.has(beat.module ?? "") || Boolean(beat.assetId || beat.assetPath || beat.footage);
    ...
    if (!existsSync(asset.absolute)) {
      issues.push(`beat ${beat.n} (${beat.name}) asset not found: ${asset.path}`);
    }
  }
  return issues;
};
