...
export const FootageScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => {
  ...
  const dur = Math.max(1, Number(beat.end) - Number(beat.start));
...
export const EvidenceScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => { const frame = useCurrentFrame(); const n = textOf(beat); return <AbsoluteFill style={{ background: C.paper, color: C.ink }}><div style={{ position: "absolute", left: "7%", right: "7%", top: "13%", borderTop: `2px solid ${C.ink}`, paddingTop: 18, fontFamily: "Arial", fontSize: 22, letterSpacing: 3, textTransform: "uppercase", color: C.muted }}>EVIDENCE</div><div style={{ position: "absolute", left: "8%", right: "8%", top: "30%", fontFamily: "Arial", fontSize: Math.min(120, Math.max(52, 1500 / Math.max(8, n.length))), fontWeight: 800, lineHeight: .95 }}>{n}</div><div style={{ position: "absolute", left: "8%", bottom: "12%", width: 180, height: 6, background: C.accent, transform: `scaleX(${Math.min(1, frame / 18)})`, transformOrigin: "left" }} /></AbsoluteFill>; };
