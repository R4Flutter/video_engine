...
const textOf = (b: LongFormBeat) => b?.typography?.text || b?.visual?.reveal || b?.narrative?.reveal || b?.narrative?.question || b?.name || "";
const mediaOf = (b: LongFormBeat) => b?.visual?.assetPath || b?.visual?.asset || b?.visual?.footage || b?.footage || b?.assetPath || b?.asset || "";

export const MediaLayer: React.FC<{ beat: LongFormBeat }> = ({ beat }) => {
  const media = mediaOf(beat);
  if (!media) return null;
  const src = String(media).replace(/^assets[\\/]/, "assets/").replace(/\\/g, "/");
  const isVideo = /\.(mp4|webm|mov|m4v)$/i.test(src);
  return isVideo ? (
    <Video src={staticFile(src)} muted loop startFrom={0} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
  ) : (
...
export const FootageScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => {
  ...
  const dur = Math.max(1, beat.end - beat.start);
  const p = frame / Math.max(1, dur * fps);
  const scale = interpolate(p, [0, 1], [1.02, 1.07], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return <AbsoluteFill style={{ background: C.bg }}><MediaLayer beat={beat} /><AbsoluteFill style={{ transform: `scale(${scale})`, transformOrigin: "50% 50%" }} /><Vignette /></AbsoluteFill>;
};

export const EvidenceScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => {
  const frame = useCurrentFrame();
  const n = textOf(beat);
  return <AbsoluteFill style={{ background: C.paper, color: C.ink }}><div style={{ position: "absolute", left: "7%", right: "7%", top: "13%", borderTop: `2px solid ${C.ink}`, paddingTop: 18, fontFamily: "Arial", fontSize: 22, letterSpacing: 3, textTransform: "uppercase", color: C.muted }}>EVIDENCE</div><div style={{ position: "absolute", left: "8%", right: "8%", top: "30%", fontFamily: "Arial", fontSize: Math.min(120, 1500 / Math.max(8, n.length)), fontWeight: 800, lineHeight: .95 }}>{n}</div><div style={{ position: "absolute", left: "8%", bottom: "12%", width: 180, height: 6, background: C.accent, transform: `scaleX(${Math.min(1, frame / 18)})`, transformOrigin: "left" }} /></AbsoluteFill>;
};

export const StatScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => <AbsoluteFill style={{ background: C.paper, color: C.ink }}><div style={{ position: "absolute", left: "8%", top: "12%", fontFamily: "Arial", fontSize: 20, letterSpacing: 4, color: C.muted }}>THE NUMBER</div><div style={{ position: "absolute", left: "8%", top: "28%", width: "84%", fontFamily: "Arial", fontSize: 132, fontWeight: 800, letterSpacing: -5 }}>{textOf(beat)}</div><div style={{ position: "absolute", left: "8%", right: "8%", bottom: "18%", height: 2, background: C.line }} /></AbsoluteFill>;
...
export const ChartScene: React.FC<{ beat: LongFormBeat }> = ({ beat }) => { const frame = useCurrentFrame(); const bars = [0.24,0.42,0.58,0.71,0.86]; return <AbsoluteFill style={{ background: C.paper, color: C.ink }}><div style={{ position:"absolute",left:"8%",top:"10%",fontFamily:"Arial",fontSize:18,letterSpacing:4,color:C.muted }}>TREND</div><div style={{ position:"absolute",left:"10%",right:"10%",bottom:"17%",height:"58%",display:"flex",alignItems:"flex-end",gap:22,borderBottom:`2px solid ${C.ink}`,borderLeft:`2px solid ${C.ink}`,padding:"0 18px" }}>{bars.map((v,i)=><div key={i} style={{ flex:1,height:`${v*100}%`,background:i===bars.length-1?C.accent:C.ink,transform:`scaleY(${Math.min(1,(frame-(i*10))/18)})`,transformOrigin:"bottom" }} />)}</div><div style={{ position:"absolute",left:"10%",top:"21%",fontFamily:"Arial",fontSize:44,fontWeight:700,maxWidth:"78%" }}>{textOf(beat)}</div></AbsoluteFill> };
...
export const StrictFallback: React.FC<{ beat: LongFormBeat }> = ({ beat }) => <AbsoluteFill style={{ background:C.paper,color:C.ink,justifyContent:"center",alignItems:"center",fontFamily:"Arial" }}><div style={{fontSize:56,fontWeight:700}}>UNMAPPED VISUAL MODULE</div><div style={{marginTop:18,fontSize:30,color:C.muted}}>{String(beat?.visual?.module || "unknown")}</div></AbsoluteFill>;
