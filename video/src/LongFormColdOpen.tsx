...
export const LongFormColdOpen: React.FC<{ enabled?: boolean }> = ({ enabled = true }) => {
  const frame = useCurrentFrame(); const { fps, width, height } = useVideoConfig(); if (!enabled) return null;
  const t = frame / fps;
  ...
  return <AbsoluteFill style={{ pointerEvents: "none" }}>
    {t >= EVIDENCE_START && t < 10 ? <AbsoluteFill style={{ background: "rgba(3,14,16,.62)", opacity: evidenceOpacity }} /> : null}
    {t >= EVIDENCE_START && t < 10 && evidenceSrc ? <div style={{ position: "absolute", left: "9%", right: "9%", top: "13%", bottom: "17%", background: "#fff", boxShadow: "0 28px 80px rgba(0,0,0,.34)", opacity: evidenceOpacity, overflow: "hidden" }}><Img src={staticFile(evidenceSrc)} style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div> : null}
    {t >= EVIDENCE_START && t < 10 ? <div style={{ position: "absolute", left: "8%", bottom: "10%", color: "white", opacity: evidenceOpacity }}><div style={{ fontFamily: theme.vox.font, fontSize: Math.min(120, width * .09), fontWeight: 900, lineHeight: .9, letterSpacing: "-.04em" }}>{metric}</div><div style={{ marginTop: 10, fontFamily: theme.vox.font, fontSize: 20, fontWeight: 800, letterSpacing: ".18em", color: theme.color.gold }}>{label}</div></div> : null}
    ...
    {t < claimIn && t >= 10 ? <div style={{ position: "absolute", left: "8%", right: "8%", bottom: "10%", color: "white", opacity: interpolate(t, [10, 10.3], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}><div style={{ fontFamily: theme.vox.font, fontSize: Math.min(54, width * .045), fontWeight: 800, lineHeight: 1.05 }}>A GYM ONLY WORKS BECAUSE MOST OF THE PEOPLE PAYING ARE SOMEWHERE ELSE.</div></div> : null}
    {src && t < OPEN_HOLD ? <Img src={staticFile(src)} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: .01 }} /> : null}
