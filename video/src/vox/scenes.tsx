...
const Stage: React.FC<{ children: React.ReactNode; align?: "center" | "flex-start" }> = ({
  ...
  const { height, pad } = useMargin();
  return (
    <AbsoluteFill
      style={{
        padding: pad,
        // Room the caption card lives in, reserved in every module so nothing
        // has to know whether this beat is captioned. This has to match where
        // <Captions> actually draws — theme.safe.bottom — or staged content
        // creeps under the narration. It used to reserve pad*2.6 (210px)
        // against a caption sitting at 192px, which only held because both were
        // wrong in the same direction.
        paddingBottom: height * (theme.safe.bottom / 1920) * 0.9,
        display: "flex",
...
export const Compare: React.FC<VoxSceneProps> = ({ dur, beat }) => {
  ...
  const rows = beat.data && beat.data.length ? beat.data : [];
  const w = width - pad * 2;
  const barH = height * 0.062;
  const step = barH * 2.5;
  const first = height * 0.38;
  const top = Math.max(...rows.map((r) => r.value), 1);

  // How much room the value beside the longest bar actually needs.
  //
  // This was a flat `w * 0.76`, chosen by eye. It is not enough: "$525,000" set
  // at barH*0.72 runs about 330px, the track ended at 779px, and the label
  // therefore started past 800px on a 1080px canvas and lost its last two
  // glyphs off the right edge. Every wide bar in the video was rendering as
  // "$525,0(".
  //
  // Measure instead. Archivo 800 in tabular figures advances ~0.58em, and the
  // widest label is the one that has to fit — so the track is whatever is left
  // once that label and its gutter have been paid for.
  // Sized against the bar, but not so large that the number is as wide as the
  // quantity it labels — at 0.72 an eight-glyph value claimed 400px and left the
  // longest bar shorter than its own caption, which inverts the comparison.
  const valueSize = barH * 0.62;
  const gutter = width * 0.022;
  const widest = Math.max(
    ...rows.map(
      (r) =>
        num(r.value, affix(r.raw ?? "").prefix, affix(r.raw ?? "").suffix, decimalsIn(r.raw ?? ""))
          .length,
    ),
    1,
  );
  const valueW = widest * valueSize * 0.58;
  // Never let the reserved column eat the bars themselves: below about half the
  // page the comparison stops being readable as a comparison.
  const track = Math.max(w * 0.42, w - valueW - gutter);
      ...
      {rows.map((row, i) => {
        ...
        return (
          <React.Fragment key={i}>
            ...
            >
              {num(row.value * grow, prefix, suffix, decimalsIn(row.raw ?? ""))}
            </div>
          </React.Fragment>
        );
      })}
...
export const Stat: React.FC<VoxSceneProps> = ({ dur, beat }) => {
  ...
  const shown = num(row.value * roll, prefix, suffix, decimalsIn(row.raw ?? ""));
          ...
          <React.Fragment key={i}>
            ...
            >
              <span style={{ color: vox.muted }}>
                {num(row.value, prefix, suffix, decimalsIn(row.raw ?? ""))}
              </span>
              <br />
              {row.label}
            </div>
