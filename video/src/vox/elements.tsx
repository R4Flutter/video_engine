...
export const KineticText: React.FC<{
  ...
  emphasis?: RegExp;
  align?: "left" | "center";
}> = ({ words, t, mode = "caption", emphasis = STRESS, align = "center" }) => {
  const { fps, width } = useVideoConfig();
  const hero = mode === "hero";
  const groups = React.useMemo(() => phrases(words, hero ? 6 : 4), [words, hero]);
  if (!groups.length) return null;

  // Which phrase owns the frame.
  //
  // This used to be `groups.find(g => t < g.last.end)` — the first group that
  // has not finished being spoken. That is wrong in the gap between two
  // phrases: the moment phrase N's last word ends, phrase N+1 becomes active,
  // but none of its words have *started*, so every one of them springs from
  // opacity 0 and the frame is blank until the speaker draws breath.
  //
  // Those gaps are not rare. Across this script they totalled 7.9s of a 37s
  // video — on a kinetic beat that is the whole frame empty, mid-sentence.
  //
  // A phrase now holds the frame until the next one actually begins. The read
  // is unchanged; the page simply stops going blank between clauses.
  let active = groups[0];
  for (const g of groups) {
    if (g[0].start <= t) active = g;
  }
  // A caption clears once the beat stops talking; a hero holds its last phrase.
  if (!hero && t >= active[active.length - 1].end && active === groups[groups.length - 1]) {
    return null;
  }
  ...
  // A caption is read on a phone held at arm's length, often at a glance while
  // the eye is really on the chart. 0.048 of the width is 52px — small enough
  // that it was decoration rather than a second channel. 0.062 is the smallest
  // size that survives that viewing distance.
  const size = hero
        ...
        return (
          <span
            ...
            style={{
              ...
              color: hit ? vox.accent : speaking && !hero ? vox.accent : vox.ink,
            }}
          >
            {word.w}
          </span>
 ...
 *
 * Every value here is mid-animation on most frames — a bar growing, a stat
 * rolling — so this formatter is read far more often at 0.6 of its value than
 * at its final one. Two fraction digits meant those intermediate frames
 * rendered "$95,999.48" and "428,233.48" and "8.13": the viewer sees cents on
 * a figure the script wrote as a round number, and it reads as a spreadsheet
 * that leaked rather than a number that was designed.
 *
 * So: whole units, unless the script itself wrote a decimal. "0.4%" keeps its
 * decimal because the author put it there; $525,000 never grows one.
 */
export const num = (value: number, prefix = "", suffix = "", decimals = 0) =>
  prefix +
...
/** How many decimals the script wrote for this quantity. The raw string is the
 *  author's intent; the animation must not invent precision beyond it. */
export const decimalsIn = (raw: string) => {
  const m = /\.(\d+)/.exec(raw ?? "");
...
export const InkChart: React.FC<{
  ...
  labels?: string[];
  unit?: string;
}> = ({ data, x, y, w, h, progress, labels, unit = "" }) => {
  const { width } = useVideoConfig();
    ...
    >
      <defs>
        <linearGradient id={`fill${id}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={vox.accent} stopOpacity={0.22} />
          <stop offset="100%" stopColor={vox.accent} stopOpacity={0} />
        </linearGradient>
        <clipPath id={`clip${id}`}>
      ...
      <line x1={0} y1={h} x2={w} y2={h} stroke={vox.ink} strokeWidth={sw * 0.5} />
      {pts.map(([px], i) => (
        <line
          key={i}
          x1={px}
      ...
      {progress > 0.02 ? (
        <>
          <circle cx={head[0]} cy={head[1]} r={sw * 1.1} fill={vox.accent} />
        ...
        ? labels.map((label, i) =>
            i < pts.length ? (
              <text
                ...
                fontWeight={700}
                fontSize={width * 0.026}
                fill={i <= headIndex ? vox.ink : vox.muted}
...
// ------------------------------------------------------------------ image bed
/**
 * Foreground b-roll, driven by voice alignment: each shot in the manifest
 * slides in from the left edge and settles right of centre, over the fixed
 * paper background. The background never changes — PaperBG is always mounted
 * underneath, and these plates are transparent PNGs that ride above the page.
 */
...
export const ImageBed: React.FC = () => {
  ...
  // Only the shot whose window we are in gets mounted.
  const i = BED.findIndex((s) => t < s.end);
  if (i < 0) return null;
  const shot = BED[i];

  // Slide: off-screen left at the shot's start, resting right-of-centre by
  // its end, eased out so the arrival lands instead of rolling to a stop.
  const p = interpolate(t, [shot.start, shot.end], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const ease = Easing.out(Easing.cubic)(p);
  // The plates are portrait crops. On a portrait canvas they scale with the
  // width and sit in the lower half; on a wide canvas they scale with the
  // height so a plate never overflows the frame vertically.
  const wide = width > height;
  const w = wide ? height * 0.42 : width * 0.56;
  const x = interpolate(ease, [0, 1], [-w * 1.25, wide ? width * 0.2 : width * 0.14]);
  const top = wide ? height * 0.14 : height * 0.26;
    ...
    <AbsoluteFill style={{ pointerEvents: "none" }}>
      <Img
        ...
        style={{
          ...
          width: w,
          height: "auto",
          // The transparent PNGs are generated on the locked palette; a light
          // grade keeps them sitting with the paper instead of on top of it.
...
// ------------------------------------------------------------------ captions
/**
 * Narration captions, ported from the stick engine (Captions.tsx).
 *
 * Word-timed against voice.json. The active word is *lifted* rather than
 * recoloured — a colour change on a moving eye reads as flicker at 30fps, a
 * position change does not. The whole line holds its place, so nothing
 * reflows under the reader.
 */
export const LiftCaption: React.FC<{
  words: Word[];
  ...
  size?: number;
  perLine?: number;
}> = ({ words, t, color = vox.ink, size = 0, perLine = 4 }) => {
  if (!words.length) return null;

  const idx = words.findIndex((w) => t >= w.start && t < w.end);
  const cur = idx === -1 ? (t < words[0].start ? 0 : words.length - 1) : idx;
  const group = Math.floor(cur / perLine);
  const slice = words.slice(group * perLine, group * perLine + perLine);
  ...
  return (
    ...
    >
      {slice.map((w, i) => {
        const active = group * perLine + i === cur;
        return (
          <span
            key={i}
            style={{
              color,
              transform: active ? `translateY(${-size * 0.09}px)` : "none",
              display: "inline-block",
            }}
          >
            {w.w}
