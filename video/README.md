...
npm run footage      # fetch requested B-roll from Pexels + Pixabay
...
The engine now treats physical B-roll as a **video-only asset class**. You do
not need to generate those clips yourself.

A beat becomes a stock-video beat when its module is `footage`, or when its
script JSON contains a `Footage`/`footage` or `footage_query` value. The fetcher
queries **both Pexels and Pixabay**, scores the returned clips for orientation,
resolution and duration, downloads the best local MP4, and records the provider,
source URL, creator, query, dimensions and duration in `src/footage.json`.

If `footage_query` is present it is preferred as the search phrase. Otherwise the
fetcher derives a query from the beat's `Footage`, `Visual`, `Reveal` and `VO`
fields. This keeps the asset workflow automatic while still allowing a precise
stock-search phrase when needed.

Set either or both API keys. With both configured, both libraries are searched
...
`--force` replaces existing downloaded clips:

```console
python tools/fetch-footage.py --force
```

API responses are cached for 24 hours under `video/out/footage-cache/`, so
re-running the stage does not repeatedly hammer either service. Existing local
clips are also skipped unless `--force` is used.

Pexels currently documents a free video-search API with default limits of 200
requests/hour and 20,000/month; it asks API users to provide prominent Pexels
attribution and credit photographers when possible. Pixabay documents a video
API with a 100-requests-per-60-seconds default limit and asks applications to
show where results come from when search results are displayed. The engine
therefore preserves provider/source metadata instead of treating downloaded
clips as anonymous assets.

### Imagery

The user-generated image workflow stays separate. `doodle`/graphic/document
assets can continue to be supplied or generated manually; the automatic stock
fetcher is intentionally **video-only**.
...
| `src/footage.json` | provider/source metadata and local clips — generated by `fetch-footage.py` |
...
| `tools/fetch-footage.py` | dual-provider Pexels + Pixabay video sourcing |
...
`tools/parse-script.mjs` (under `finance` or `vox`), and add a component under the
same key in the matching `scenes.tsx`. Nothing else needs to know about it.
