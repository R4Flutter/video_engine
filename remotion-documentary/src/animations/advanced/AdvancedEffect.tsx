...
const AdvancedText: React.FC<{ effect: string; text?: string; config: Config; t: number; intensity: number }> = ({ effect, text = "DOCUMENTARY", config, t, intensity }) => {
  ...
  let weight = Math.round(num(config.fontWeight, effect === "fullScreenStatement" ? 800 : 700));
  ...
  if (effect === "textFade") opacity = p;
  if (effect === "textSlide") transform = `translate(${interpolate(p, [-1, 0], [-100, -50])}%, -50%)`;
  ...
  if (effect === "splitTypography" || effect === "textCollision" || effect === "textStacking") {
    const yShift = interpolate(easeOutExpo(clamp01(p)), [0, 1], [80, 0]);
    transform = `translate(-50%, calc(-50% + ${yShift}px))`;
  }
  if (effect === "letterSpacingAnimation" || effect === "trackingAnimation") {
    letterSpacing = `${interpolate(p, [0, 1], [0.55, 0.08])}em`;
  }
  ...
  if (effect === "highlightSweep") clipPath = `inset(0 ${100 - p * 100}% 0 0)`;
  ...
  return (
    <div style={{
      position: "absolute", left: `${x}%`, top: `${y}%`, color, fontFamily: family,
      fontSize, fontWeight: weight, lineHeight: 0.95,
      textAlign: (config.textAlign as React.CSSProperties["textAlign"]) ?? "center",
      letterSpacing, opacity, filter, transform, textDecoration: decoration,
      whiteSpace: "pre-wrap", maxWidth: "92%", clipPath,
      ...
      {effect === "highlightSweep" ? <span style={{position: "absolute", left: 0, top: 0, width: "100%", height: "100%", background: str(config.highlight, "rgba(255,215,64,.32)"), mixBlendMode: "screen", pointerEvents: "none"}} /> : null}
    </div>
  );
};

export const AdvancedEffect: React.FC<BaseEffectProps & { config?: Config; effect?: string }> = ({
  image, children, durationInFrames = 90, delay = 0, intensity = 1,
  style, className, config = {}, effect = "",
}) => {
  ...
  const camera = (): React.ReactNode => {
    ...
    else if (effect === "wideShot") scale = interpolate(p, [0, 1], [1.12, 1]);
    ...
    return <Img src={src ?? ""} style={imageStyle(`translate(${x}%, ${y}%) scale(${scale}) rotate(${rotate}deg)`)} />;
  };

  const transitionNames = new Set(["crossfade", "hardCut", "matchCut", "transitionDipToBlack", "transitionDipToWhite", "whipPan", "zoomTransition", "blurTransition", "lightFlash", "filmBurn", "paperWipe", "newspaperWipe", "documentWipe", "shapeWipe", "maskTransition", "imageMorph", "filmBurnReveal", "lightLeakReveal"]);
  const textNames = new Set(["textFade", "textSlide", "textRise", "textDrop", "textScale", "textBlurSharp", "typewriter", "charByChar", "wordByWord", "lineByLine", "textMaskReveal", "textWipeReveal", "kinetic", "wordEmphasis", "numberEmphasis", "oversizedNumber", "fullScreenStatement", "splitTypography", "trackingAnimation", "letterSpacingAnimation", "weightChange", "highlightSweep", "underlineDraw", "strikeThrough", "textDisplacement", "textStagger", "textStacking", "textCollision", "textSnapping"]);
  ...
  if (textNames.has(effect)) return <AbsoluteFill style={style} className={className}><AdvancedText effect={effect} text={str(config.text, str(config.label, "DOCUMENTARY"))} config={config} t={t} intensity={intensity} />{children}</AbsoluteFill>;

  if (transitionNames.has(effect)) {
    const wipe = clamp01(t);
    const isDip = effect === "transitionDipToBlack" || effect === "transitionDipToWhite";
    const dipWhite = effect === "transitionDipToWhite";
    const nextOpacity = nextImage && effect !== "hardCut" ? wipe : 0;
    const transform = effect === "whipPan" ? `translateX(${interpolate(p, [0, 1], [0, -18])}%)` : effect === "zoomTransition" ? `scale(${interpolate(p, [0, .8, 1], [1, 1.48, 1])})` : "scale(1)";
    const oldStyle: React.CSSProperties = imageStyle(transform, {opacity: nextImage ? 1 - nextOpacity : 1});
    ...
    return <AbsoluteFill style={style} className={className}>
      ...
      {isDip ? <div style={{position:"absolute", inset:0, background: dipWhite ? "#fff" : "#000", opacity: interpolate(p,[0,.5,1],[0,1,0])}} /> : null}
  ...
  if (focusNames.has(effect)) {
    ...
    const size = num(config.size, effect === "circularSpotlight" ? .34 : .42);
    return <AbsoluteFill style={style} className={className}>
      ...
      {effect === "outlineSubject" ? <div style={{position:"absolute", left:`${target.x*100}%`, top:`${target.y*100}%`, width:`${size*100}%`, height:`${size*100}%`, transform:"translate(-50%,-50%)", border:`3px solid ${accent}`, borderRadius:20, boxShadow:`0 0 28px ${accent}99`, opacity:p}} /> : null}
  ...
  if (mapNames.has(effect)) {
    ...
    return <AbsoluteFill style={style} className={className}>
      ...
      {effect !== "countryHighlight" && effect !== "cityHighlight" ? <svg width={width} height={height} style={{position:"absolute", inset:0, overflow:"visible"}}><path d={d} fill="none" stroke={accent} strokeWidth={4} strokeLinecap="round" strokeDasharray="12 10" strokeDashoffset={interpolate(p,[0,1],[220,0])} opacity={effect === "routeDrawing" || effect === "flightPath" || effect === "connectionLines" ? 1 : .35} /></svg> : null}
  ...
  if (["faceCrop","objectCrop","documentCrop","logoCrop","moneyCrop","computerCrop","graphCrop","backgroundCrop","extremeCloseUp","wideShot","mediumShot","detailShot"].includes(effect)) {
    ...
    return <AbsoluteFill style={style} className={className}><Img src={src ?? ""} style={imageStyle(`translate(${tx}%, ${ty}%) scale(${interpolate(p,[0,1],[1,crop])})`)} />{children}</AbsoluteFill>;
  }

  return <AbsoluteFill style={style} className={className}>{camera()}{children}</AbsoluteFill>;
};