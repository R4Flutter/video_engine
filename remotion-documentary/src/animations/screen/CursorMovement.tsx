...
export interface CursorMovementProps extends BaseEffectProps {
  region?: { x?: number; y?: number; width?: number; height?: number };
  path?: Array<{ x: number; y: number }>;
  intensity?: number;
}

export const CursorMovement: React.FC<CursorMovementProps> = ({
  image,
  children,
  region = { x: 0, y: 0, width: 100, height: 100 },
  path: pathPoints = [
    { x: 20, y: 80 }, { x: 40, y: 40 }, { x: 60, y: 70 }, { x: 80, y: 30 },
  ],
  intensity = 1,
  style,
  className,
